 ...
 * 
 * Based on code taken from MoveAction
 */
public class OrientedMoveAction extends AbstractAction {
	/**
	 ...
	 */
	public OrientedMoveAction(Toroidal2DPhysics space, Position currentLocation, 
			Position targetLocation, Vector2D targetVelocity, double targetOrientation) {
		super();
...
//		float Kp = 0.16f;
	...
	/**
	 * Returns the error in orientation space using the target orientation
	 * (separate function so we can use it to decide when it has reached the goal)
	 * 
	 * @param space
	 * @param currentLoc
	 * @return
	 */
	private double getTargetOrientationError(Toroidal2DPhysics space, Position currentLoc) {
		double error = (targetOrientation - currentLoc.getOrientation());
